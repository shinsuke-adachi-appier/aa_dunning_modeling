...
- **Source:** BigQuery view `aa-datamart.billing_dm.MISc_vw_txn_enriched_subID_fallback` (via `txn_pipeline.run_pipeline()`). Raw data can be cached to `data/dunning_raw.parquet`; set `FORCE_QUERY=1` to refresh.
