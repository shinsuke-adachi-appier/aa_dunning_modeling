...
- **Metric:** For each decile, **actual recovery rate** (mean of `recovered`). The **Top Decile Lift Factor** = (Top decile success rate) / (Global success rate). A lift > 1 indicates the highest-probability segment contains more successes than average (lift curve).
...
- **Definition:** Among **recovered** invoices, the share where |`hours_suggested_to_recovered`| ≤ 6 (recovery within ±6 h of the model’s #1 slot).
...
1. **Data:** Unique invoices shadowed; total shadow rows; rows with suggestion; actual outcomes matched.
2. **Dual-path evaluation:** Overall recovery rate and TTR lift for **Initial Intent** and **Last-Mile Accuracy**.
3. **Recoveries (Last-Mile):** Total recoveries and overall recovery rate.
4. **Relative opportunity gap:** Dynamic threshold (BASELINE_LIFT_FACTOR × global rate); count of high-value opportunities missed by Chargebee.
5. **Top-1 performance:** % of recoveries within ±6 h of the model’s #1 slot.
6. **TTR lift:** Mean (recovered_at − suggested_optimal_retry_at) in hours, **valid TTR only** (recovered_at ≥ inference_run_at).
7. **Decile calibration:** Table of actual recovery rate (and mean predicted P) per decile; **Top Decile Lift Factor**.
8. **EV lift:** Total potential revenue lift (sum of amount × (P − baseline)).
9. **Transition matrix by decline code:** Recovery rate and mean TTR per `prev_decline_code`.

### Comparison CSV: `artifacts/shadow_vs_actual_comparison.csv`

- Based on the **Last-Mile** merge (latest record per invoice). Columns include: shadow fields, `recovered`, `recovered_at`, `last_attempt_at`, `hours_suggested_to_recovered`, `high_prob_missed_by_chargebee`, `high_prob_threshold_used`, `decile`, `recovered_within_top1_window`, `invoice_amount`, `ev_lift`, `prev_decline_code`, `prev_card_status`. The latter two come from actuals (BQ "previous" attempt) when available, else from the shadow log’s `raw_features_snapshot`.
