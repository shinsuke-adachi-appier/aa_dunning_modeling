...
# For joblib.load: model was pickled with __main__.IsotonicCalibratedClassifier (from train script).
# We must register the same class on current __main__ before loading.
...
def fetch_active_dunning_invoices() -> pd.DataFrame:
    ...
    try:
        ...
        from src.data.timezone import add_timezone_features
    except Exception as e:
        print(f"Error importing src.data: {e}", file=sys.stderr)
        raise
    try:
...
def load_calibrated_model(model_path: str) -> Any:
    ...
    # Ensure __main__ has IsotonicCalibratedClassifier so joblib can unpickle (model was saved from train script).
    if not hasattr(sys.modules["__main__"], "IsotonicCalibratedClassifier"):
...
def run_inference_for_invoice(
    ...
    """
    from src.evaluation.ranking_backtest import optimal_slot_for_invoice
...
def run_shadow_monitoring(
    ...
    if slot_path and slot_rows_all:
        ...
        if not slot_path_p.is_absolute():
            slot_path_p = PROJECT_ROOT / slot_path_p
...
def main() -> None:
    ...
    model_path = os.environ.get("DUNNING_MODEL_PATH", str(PROJECT_ROOT / "models" / DEFAULT_CALIBRATED_MODEL))
    model_version_id = os.environ.get("DUNNING_MODEL_VERSION", None)
    output_path = os.environ.get("SHADOW_LOG_PATH", str(PROJECT_ROOT / "artifacts" / "shadow_log.csv"))
